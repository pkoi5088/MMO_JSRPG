...
namespace Server.Game.Room
{
    public class VisionCube
    {
        ...
        public HashSet<GameObject> PreviouseObjects { get; private set; } = new HashSet<GameObject>();
        ...
        public void Update()
        {
            ...
            List<GameObject> added = currentObjects.Except(PreviouseObjects).ToList();
            if (added.Count > 0)
            {
            ...
            // 기존엔 있없는데 사라진 애들 Despawn 처리
            List<GameObject> removed = PreviouseObjects.Except(currentObjects).ToList();
            if (removed.Count > 0)
            {
            ...
            PreviouseObjects = currentObjects;
